...
class MDTv2(nn.Module):
    ...
    def __init__(
        ...
        # self.y_embedder = nn.Linear(num_classes, hidden_size)
        self.fourier_feats = nn.Sequential(SinusoidalEmbedding(embedding_dims=noise_embed_dims),
                                           nn.Linear(noise_embed_dims, hidden_size),
                                           nn.GELU(),
                                           nn.Linear(hidden_size, hidden_size)
                                           )
        self.label_proj = nn.Linear(text_emb_size, hidden_size)
    ...
    def fix_masking(self, x, mask_token,c=None):
        ...
        if c is not None:
            ...
            return x_masked, mask_token, ids_restore, ids_keep
    ...
    def forward(self, x, noise_level, y, mask_token=None,enable_mask=False):
        """
        Forward pass of MDT.
        ...
        if mask_token is not None:
            x, mask, ids_restore, ids_keep = self.fix_masking(
                x, mask_token)
            masked_stage = True

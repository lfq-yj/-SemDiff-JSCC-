...
if __name__ == '__main__':
    ...
    if not os.path.exists(base_root):
        os.makedirs(base_root, exist_ok=True)  # 递归创建所有上级目录，已存在则不报错
    ...
    if config.model.condition_setting.use_semantic:
        ...
        processor = AutoProcessor.from_pretrained("./blip2-processor/")
        
        caption_model = Blip2ForConditionalGeneration.from_pretrained("./blip2-processor/", torch_dtype=torch.float16,local_files_only=True )
